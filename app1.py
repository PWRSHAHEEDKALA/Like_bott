...
# List of all remain files to reset
REMAIN_FILES = [
    "ind_remain.json",
    "bd_remain.json",
    "br_remain.json",
...
# Function to reset all remain files to 0
def reset_all_remain_files():
    while True:
        now = datetime.now()
        # Check if it's 4 AM
        if now.time() >= dt_time(4, 0) and now.time() <= dt_time(4, 5):
            app.logger.info("4 AM detected - Resetting all remain files...")
            for file_path in REMAIN_FILES:
                new_data = json.dumps({"counter": 0})
                if update_file_on_github(file_path, new_data):
                    app.logger.info(f"Successfully reset {file_path}")
                else:
                    app.logger.error(f"Failed to reset {file_path}")
            # Sleep for 5 minutes to avoid multiple resets in the same minute
            time.sleep(300)
        # Sleep for 1 minute before checking again
        time.sleep(60)

# Start the reset thread when the app starts
reset_thread = threading.Thread(target=reset_all_remain_files)
...
def load_tokens(server_name):
    try:
        ...
        elif server_name in {"ME", "EU", "SG", "NA"}:
